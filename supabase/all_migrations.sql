...
-- 20260625161200: Fix mirror trigger guest_id FK violation
-- guest_id FK references crm_guests(id) — guest users don't have a
-- matching crm_guests row, causing FK violation that rolls back the
-- entire guest_bookings insert transaction.
-- Fix: set guest_id to NULL for portal bookings.
CREATE OR REPLACE FUNCTION public.mirror_guest_booking_to_reservation()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
