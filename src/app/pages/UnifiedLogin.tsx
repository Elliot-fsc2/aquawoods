    ...
    <div className="min-h-screen grid md:grid-cols-2">
      ...
      <div className="relative hidden md:flex items-center justify-center overflow-hidden bg-brand-900 text-cream-50 p-12">
        <div
          ...
          style={{ backgroundImage: "url(https://images.unsplash.com/photo-1571896349842-33c89424de2d?auto=format&fit=crop&w=1200&q=80)" }}
        />
          ...
          {isGuest ? (
            <>
              ...
              <p className="text-cream-100/80 mb-8 leading-relaxed">
                Manage bookings, order food to your room, request services, earn loyalty rewards, and check in faster with your own guest account.
              </p>
              <div className="space-y-3 text-sm text-cream-100/70">
                {[
                  ...
                  <div key={f} className="flex items-center gap-2"><span className="w-1.5 h-1.5 bg-gold-400 rounded-full" /> {f}</div>
                ))}
              </div>
            </>
          ) : (
            <>
              <h1 className="font-serif text-5xl mb-6 leading-tight">Hospitality Sales Management System</h1>
              <p className="text-cream-100/80 mb-8 leading-relaxed">
                The unified command center for Aquawood Garden Resort — front desk, revenue management, channel distribution, MICE sales, CRM, and executive reporting in one platform.
              </p>
              <div className="space-y-3 text-sm text-cream-100/70">
                {["Real-time inventory & folio management", "Dynamic pricing & revenue forecasting", "Multi-channel distribution sync", "Guest CRM & loyalty analytics"].map((f) => (
                  <div key={f} className="flex items-center gap-2"><Shield className="w-4 h-4 text-gold-400" /> {f}</div>
                ))}
              </div>
            </>
          )}
      ...
      <div className="flex items-center justify-center p-8 bg-cream-50">
        <div className="w-full max-w-md">
          <a href="/" className="inline-flex items-center gap-2 text-sm text-brand-700 hover:text-brand-900 mb-8">
          ...
          <div className="flex border-b border-brand-200 mb-8">
            ...
            <button
              onClick={() => { setTab("guest-login"); setError(""); }}
          ...
          {isGuest && (
            <div className="flex gap-4 mb-6">
              ...
              <button
                onClick={() => { setTab("guest-register"); setError(""); }}
          ...
          {tab === "staff" && (
            <>
              ...
              <form onSubmit={handleStaffLogin} className="space-y-4">
                <div>
                  <label className="text-xs uppercase tracking-wider text-brand-700 font-medium">Email</label>
                ...
                <div>
                  <label className="text-xs uppercase tracking-wider text-brand-700 font-medium">Password</label>
                  <div className="relative mt-1">
                ...
                <button disabled={loading} className="w-full py-3 bg-brand-800 text-cream-50 rounded-md hover:bg-brand-900 transition font-medium disabled:opacity-60">
            ...
            <>
              ...
              <form onSubmit={handleGuestLogin} className="space-y-4">
                <div>
                  ...
                  <div className="relative mt-1">
                    <Mail className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-brand-400" />
                    <input type="email" required value={guestEmail} onChange={(e) => setGuestEmail(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-brand-200 rounded-md focus:outline-none focus:border-brand-500 bg-white" />
                  </div>
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-brand-700 font-medium">Password</label>
                  <div className="relative mt-1">
                    <Lock className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-brand-400" />
                    <input type="password" required value={guestPass} onChange={(e) => setGuestPass(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-brand-200 rounded-md focus:outline-none focus:border-brand-500 bg-white" />
                  </div>
                </div>
                {error && <div className="text-sm text-red-600 bg-red-50 px-3 py-2 rounded">{error}</div>}
              ...
              <div className="mt-6 pt-6 border-t border-brand-100 text-xs text-brand-600">
                ...
                <div className="font-mono bg-brand-50 rounded p-2">
          ...
          {tab === "guest-register" && (
            <>
              ...
              <p className="text-brand-700 text-sm mb-6">Start earning loyalty points with your first stay.</p>
              <form onSubmit={handleRegister} className="space-y-4">
                <div>
                  <label className="text-xs uppercase tracking-wider text-brand-700 font-medium">Full Name</label>
                  <div className="relative mt-1">
                    <User className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-brand-400" />
                    <input required value={regName} onChange={(e) => setRegName(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-brand-200 rounded-md focus:outline-none focus:border-brand-500 bg-white" />
                  </div>
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-brand-700 font-medium">Email</label>
                  <div className="relative mt-1">
                    <Mail className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-brand-400" />
                    <input type="email" required value={regEmail} onChange={(e) => setRegEmail(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-brand-200 rounded-md focus:outline-none focus:border-brand-500 bg-white" />
                  </div>
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-brand-700 font-medium">Mobile Number</label>
                  <div className="relative mt-1">
                    <Phone className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-brand-400" />
                    <input type="tel" required value={regPhone} onChange={(e) => setRegPhone(e.target.value)} placeholder="+63 9XX XXX XXXX" className="w-full pl-10 pr-4 py-3 border border-brand-200 rounded-md focus:outline-none focus:border-brand-500 bg-white" />
                  </div>
                </div>
                <div>
                  <label className="text-xs uppercase tracking-wider text-brand-700 font-medium">Password</label>
                  <div className="relative mt-1">
                    <Lock className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-brand-400" />
                    <input type="password" required minLength={4} value={regPass} onChange={(e) => setRegPass(e.target.value)} className="w-full pl-10 pr-4 py-3 border border-brand-200 rounded-md focus:outline-none focus:border-brand-500 bg-white" />
                  </div>
                </div>
                {error && <div className="text-sm text-red-600 bg-red-50 px-3 py-2 rounded">{error}</div>}
                <button className="w-full py-3 bg-brand-800 text-cream-50 rounded-md hover:bg-brand-900 transition font-medium flex items-center justify-center gap-2">
                ...
                <p className="text-xs text-brand-600 text-center">By creating an account, you'll receive 200 welcome loyalty points.</p>
              </form>
            </>
          )}
        </div>
