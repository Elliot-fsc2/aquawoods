...
const seedEmployees: Employee[] = [
  {
    ...
    email: "elena@aquawood.com",
    phone: "+66 81 555 0100",
    shift: "Day (9-6)",
    status: "Active",
  },
  {
  ...
  },
  {
    ...
    department: "F&B",
    email: "marco@aquawood.com",
    phone: "+66 81 555 0106",
    shift: "Day (9-9)",
    status: "Active",
  },
  {
    ...
    position: "Concierge",
    department: "Front Office",
    email: "carlos@aquawood.com",
    phone: "+66 81 555 0107",
    shift: "Evening (2-10)",
    status: "Active",
  },
];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-brand-700 mt-1">
          Manage property, rooms, employees, housekeeping, and system preferences.
        </p>
      </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-amber-800">
              Some settings require administrator privileges. Sign in as admin to make changes.
            </div>
          </div>
        </div>
      )}
...
function PropertyTab() {
  ...
  const save = () => {
    ...
    addToast(
      ...
      "All changes have been applied across the system.",
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-brand-100 p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            ...
            <div className="text-xs text-brand-600">
              Upload your property logo and browser favicon
            </div>
          </div>
        </div>
        <div className="grid md:grid-cols-2 gap-6">
          {/* LOGO */}
          <div>
            <div className="text-xs uppercase tracking-wider text-brand-700 font-medium mb-2">
              Property Logo
            </div>
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <button
                  ...
                  className="px-3 py-2 bg-brand-800 text-cream-50 rounded-md text-xs hover:bg-brand-900 flex items-center gap-1 mb-2"
                >
                ...
                <div className="text-[10px] text-brand-500 mt-1">
                  PNG/SVG recommended · Square aspect
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <input
                  ...
                  onChange={(e) => handleFile(e.target.files?.[0] || null, "favicon")}
                  className="hidden"
                />
                <button
                ...
                {draft.favicon && (
                  <button
                    onClick={() => setDraft({ ...draft, favicon: null })}
                    className="block text-xs text-red-600 hover:text-red-700"
                  >
                    Remove
                  </button>
      ...
      <div className="bg-white rounded-xl border border-brand-100 p-6">
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-brand-600">
                All fields are editable — saved to all front-end and back-end views
              </div>
            </div>
          </div>
          {saved && (
        ...
        <div className="flex justify-end gap-2 mt-6 pt-6 border-t border-brand-100">
          ...
          <button
            onClick={save}
            disabled={!isDirty}
            className="px-4 py-2 bg-brand-800 text-cream-50 rounded-md text-sm hover:bg-brand-900 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1"
          >
        ...
        <div className="flex items-start gap-4">
          {draft.logo && (
            <img
              src={draft.logo}
              alt="Logo"
              className="w-16 h-16 rounded-full object-cover bg-cream-50 border-2 border-gold-400"
            />
          )}
          <div className="flex-1">
            ...
            <div className="text-xs text-cream-100/70 mt-2">
              {draft.starRating} · {draft.type} · Est. {draft.established}
              <br />
...
function RoomsTab() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .order("name")
      .then(({ data }) => {
        if (data) setCategories(data as RoomCategory[]);
      });
  }, []);
  ...
  const [roomSort, setRoomSort] = useState<"number" | "type" | "rateAsc" | "rateDesc" | "capacity">(
    "number",
  );
  ...
  const seedSampleRooms = async () => {
    const totalSeed = seedCategories.reduce((s, c) => s + c.quantity, 0);
    const msg =
      rooms.length > 0
        ? `This will delete all ${rooms.length} existing rooms and create ${totalSeed} new sample rooms. Continue?`
        : `Generate ${totalSeed} sample rooms across ${seedFloors.length} floors?`;
    ...
    for (const cat of seedCategories) {
      const floor = seedFloors.find((f) => f.id === cat.floorId);
      const level = floor?.level ?? 1;
      const startNum = level * 100 + 1;
      for (let i = 0; i < cat.quantity; i++) {
        const roomNum = String(startNum + i);
        const res = await addRoom({
          number: roomNum,
          floor: level,
          type: cat.name as import("../data/mockData").RoomType,
          baseRate: cat.basePrice,
          status: "available",
          beds: i % 2 === 0 ? "1 King" : "2 Queen",
          capacity: cat.capacity,
          amenities: cat.amenities.split(",").map((s) => s.trim()),
          image: "",
        });
        if (res.ok) created++;
      }
    ...
    addToast(
      "success",
      `Seeded ${created} rooms`,
      "Sample rooms created with 'available' status. They are now visible on the landing page and front desk.",
    );
    setSeeding(false);
  };
  ...
  const saveCategory = async (data: Omit<RoomCategory, "id"> & { id?: string }) => {
    if (data.id) {
      ...
      setCategories((cs) =>
        cs.map((c) => (c.id === data.id ? { ...c, ...data, id: data.id! } : c)),
      );
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* EMPTY STATE — SEED ROOMS */}
      {!selectedFloor && (
        <div
          className={`rounded-xl p-6 text-center ${
            rooms.length === 0
              ? "bg-gradient-to-br from-brand-900 to-brand-800 text-cream-50"
              : "bg-white border border-brand-100"
          }`}
        >
          <Bed
            className={`w-10 h-10 mx-auto mb-3 ${
              rooms.length === 0 ? "text-gold-400" : "text-brand-400"
            }`}
          />
          <h3
            className={`font-serif text-xl mb-1 ${
              rooms.length === 0 ? "text-cream-50" : "text-brand-900"
            }`}
          >
            {rooms.length === 0 ? "No Rooms Yet" : `${rooms.length} rooms in database`}
          </h3>
          <p
            className={`max-w-lg mx-auto mb-4 text-sm ${
              rooms.length === 0 ? "text-cream-100/80" : "text-brand-600"
            }`}
          >
            {rooms.length === 0
              ? "Click below to instantly generate sample rooms based on your floor and category configuration."
              : "Reset and regenerate all rooms from your floor/category config. This will replace existing rooms."}
          </p>
          <button
            onClick={seedSampleRooms}
            disabled={seeding}
            className={`px-5 py-2.5 rounded-lg text-sm font-medium transition flex items-center gap-2 mx-auto ${
              rooms.length === 0
                ? "bg-gold-500 text-brand-900 hover:bg-gold-400"
                : "bg-brand-800 text-cream-50 hover:bg-brand-900"
          ...
          >
            ...
            {seeding
              ? "Generating rooms..."
              : rooms.length === 0
                ? `Generate ${seedCategories.reduce((s, c) => s + c.quantity, 0)} Sample Rooms`
                : `Regenerate ${seedCategories.reduce((s, c) => s + c.quantity, 0)} Sample Rooms`}
          </button>
        </div>
      )}
              ...
              {floors.map((floor) => {
                ...
                return (
                  <tr
                    key={floor.id}
                    onClick={() => {
                      setSelectedFloor(isSel ? null : floor);
                      setEditingRoom(null);
                        ...
                        <button
                          onClick={async () => {
                            if (liveCount > 0) {
                              if (
                                !confirm(
                                  `This floor has ${liveCount} room${liveCount > 1 ? "s" : ""} assigned. Deleting it will not remove the rooms from the database, but may affect floor-level organization. Proceed?`,
                                )
                              )
                                return;
                            } else {
        ...
        <div className="bg-white rounded-xl border-2 border-gold-300 p-6">
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-brand-600">
                  Level {selectedFloor.level} · {roomsOnFloor.length} room(s) · live from database
                </div>
              </div>
            </div>
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-2 border border-brand-200 rounded-md text-xs hover:bg-brand-50 flex items-center gap-1"
              >
              ...
              <button
                onClick={() => {
                  setShowBulkRoomForm(true);
                  setShowRoomForm(false);
                  setEditingRoom(null);
                }}
                className="px-3 py-2 bg-gold-500 text-brand-900 rounded-md text-xs hover:bg-gold-400 flex items-center gap-1"
              >
          ...
          {(showRoomForm || editingRoom) && (
            <RoomForm
              ...
              onSave={async (data) => {
                const res = data.id ? await updateRoom(data.id, data) : await addRoom(data);
                if (!res.ok) {
                ...
                addToast(
                  "success",
                  data.id ? "Room updated" : "Room added",
                  `Room ${data.number} on Level ${data.floor}.`,
                );
                setEditingRoom(null);
                setShowRoomForm(false);
              }}
              ...
              onSave={async (roomsToCreate) => {
                ...
                if (failed === 0) {
                  addToast(
                    "success",
                    `${created} rooms added`,
                    `All rooms created on Level ${selectedFloor.level}.`,
                  );
                } else {
                  addToast(
                    "error",
                    `${failed} failed`,
                    `${created} created, ${failed} could not be saved.`,
                  );
                }
                setShowBulkRoomForm(false);
              }}
                    ...
                    <td className="py-3">
                      {room.image ? (
                        <img
                          src={room.image}
                          alt={room.number}
                          className="w-12 h-12 rounded object-cover border border-brand-100"
                        />
                      ) : (
                        <div className="w-12 h-12 rounded bg-brand-50 border border-brand-100 flex items-center justify-center text-brand-300">
                          ...
                          onClick={async () => {
                            ...
                            if (!res.ok)
                              addToast(
                                "error",
                                "Delete failed",
                                res.error || "Could not delete room.",
                              );
                            else
                              addToast("success", "Room deleted", `Room ${room.number} removed.`);
                          }}
                          className="p-1.5 rounded hover:bg-red-50 text-red-600"
                        >
                          <Trash2 className="w-3.5 h-3.5" />
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-brand-600">
                Assign categories to floors with pricing and capacity
              </div>
            </div>
          </div>
          <button
                  ...
                  <tr key={cat.id} className="hover:bg-brand-50/40">
                    ...
                    <td className="py-3 text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <button
                          onClick={async () => {
                            await supabase.from("room_categories").delete().eq("id", cat.id);
      ...
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <label className="text-xs text-brand-700">
          ...
          <input
            ...
            type="number"
            required
            defaultValue={room?.floor ?? defaultFloor}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          />
        </label>
        <label className="text-xs text-brand-700">
        ...
        <label className="text-xs text-brand-700">
          ...
          <select
            ...
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          >
            {["available", "occupied", "dirty", "maintenance", "reserved"].map((s) => (
          ...
          <div className="flex items-center gap-3">
            {image ? (
              <img
                src={image}
                alt="preview"
                className="w-20 h-20 rounded object-cover border border-brand-200"
              />
            ) : (
              <div className="w-20 h-20 rounded bg-white border border-dashed border-brand-200 flex items-center justify-center text-brand-300 text-xs">
                No image
              </div>
            )}
            <label
              className={`px-3 py-1.5 bg-white border border-brand-200 rounded-md text-xs cursor-pointer hover:bg-brand-100 inline-flex items-center gap-1 w-fit ${uploading ? "opacity-60 pointer-events-none" : ""}`}
            >
            ...
            {image && (
              <button
                type="button"
                onClick={() => setImage("")}
                className="px-3 py-1.5 border border-red-200 text-red-600 rounded-md text-xs hover:bg-red-50 inline-flex items-center gap-1"
              >
...
function BulkRoomForm({
  ...
  const [sharedStatus, setSharedStatus] =
    useState<import("../data/mockData").RoomStatus>("available");
  ...
  return (
    <form onSubmit={handleSubmit} className="mb-4 p-4 rounded-lg bg-gold-50 border border-gold-200">
      ...
      <div className="grid md:grid-cols-4 gap-3">
        <label className="text-xs text-brand-700">
          Number Prefix <span className="text-brand-400 font-normal">(optional)</span>
          <input
            value={prefix}
        ...
        <label className="text-xs text-brand-700">
          ...
          <input
            ...
            readOnly
            value={defaultFloor}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-brand-100 text-sm text-brand-500"
          />
        </label>
        <label className="text-xs text-brand-700">
          Type
        ...
        <label className="text-xs text-brand-700">
          ...
          <input
            ...
            min={0}
            value={sharedRate}
            onChange={(e) => setSharedRate(Number(e.target.value))}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          />
        </label>
        <label className="text-xs text-brand-700">
          ...
          <input
            ...
            onChange={(e) => setSharedCapacity(Number(e.target.value))}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          />
        </label>
        <label className="text-xs text-brand-700">
          ...
          <input
            value={sharedBeds}
            onChange={(e) => setSharedBeds(e.target.value)}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          />
        </label>
        <label className="text-xs text-brand-700">
          ...
          <select
            value={sharedStatus}
            onChange={(e) =>
              setSharedStatus(e.target.value as import("../data/mockData").RoomStatus)
            }
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          >
            {["available", "occupied", "dirty", "maintenance", "reserved"].map((s) => (
        ...
        <label className="text-xs text-brand-700 md:col-span-3">
          Amenities (comma separated)
          <input
      ...
      <div className="mt-3 p-2 bg-white rounded border border-gold-100 text-xs text-brand-600">
        <span className="font-medium">Preview:</span> {generatedNumbers.slice(0, 8).join(", ")}
      ...
      <div className="flex justify-end gap-2 mt-3">
        ...
        <button
          ...
          className="px-3 py-1.5 bg-brand-800 text-cream-50 rounded-md text-xs hover:bg-brand-900 flex items-center gap-1"
        >
          <ListPlus className="w-3 h-3" /> Create {count} Rooms
        </button>
      </div>
    </form>
  );
...
function FloorForm({
  ...
  floor: Floor | null;
  onSave: (data: Omit<Floor, "id"> & { id?: string }) => void;
  onCancel: () => void;
}) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
      ...
      <div className="grid md:grid-cols-4 gap-3">
        ...
        <label className="text-xs text-brand-700">
          ...
          <input
            ...
            type="number"
            required
            defaultValue={floor?.roomCount ?? 0}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          />
        </label>
      </div>
...
function CategoryForm({
  ...
  floors: Floor[];
  onSave: (data: Omit<RoomCategory, "id"> & { id?: string }) => void;
  onCancel: () => void;
}) {
  ...
  return (
    ...
    >
      <div className="text-xs uppercase tracking-wider text-brand-600 mb-3">
        {category ? "Update" : "Create"} Room Category
      </div>
      <div className="grid md:grid-cols-3 gap-3">
        <label className="text-xs text-brand-700">
          Category Name
            ...
            <div className="flex flex-col gap-2">
              ...
              {image && (
                <button
                  ...
                  className="px-3 py-1.5 border border-red-200 text-red-600 rounded-md text-xs hover:bg-red-50 inline-flex items-center gap-1 w-fit"
                >
                  <Trash2 className="w-3 h-3" /> Remove
                </button>
              )}
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-brand-600">
                Manage staff profiles, departments, shifts, and status
              </div>
            </div>
          </div>
                ...
                <tr key={emp.id} className="hover:bg-brand-50/40">
                  <td className="py-3">
                    <input
                      ...
                      onChange={() =>
                        setSelectedIds((c) =>
                          c.includes(emp.id) ? c.filter((id) => id !== emp.id) : [...c, emp.id],
                        )
                      }
                      className="h-4 w-4 accent-brand-700"
                    />
                  ...
                  <td className="py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${emp.status === "Active" ? "bg-emerald-100 text-emerald-800" : emp.status === "On Leave" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-700"}`}
                    >
                      {emp.status}
                    </span>
                  </td>
      ...
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <label className="text-xs text-brand-700">
          ...
          <select
            name="department"
            required
            defaultValue={employee?.department || "Front Office"}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          >
        ...
        <label className="text-xs text-brand-700">
          ...
          <select
            name="shift"
            required
            defaultValue={employee?.shift || "Day (9-6)"}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          >
            <option>Morning (6-2)</option>
            <option>Afternoon (2-10)</option>
            <option>Evening (2-10)</option>
            <option>Night (10-6)</option>
            <option>Day (9-6)</option>
        ...
        if (data)
          setTasks(
            data.map((r: Record<string, unknown>) => ({
              id: r.id as string,
              roomNumber: r.room as string,
              assignedTo: r.assigned_to as string,
              taskType: r.task as string,
              priority: r.priority as HousekeepingTask["priority"],
              status: r.status as string as HousekeepingTask["status"],
              eta: r.eta as string,
            })),
          );
      });
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-brand-600">
                Assign cleaning tasks, track priorities and completion
              </div>
            </div>
          </div>
          <div className="flex gap-2 items-center">
                ...
                <tr key={task.id} className="hover:bg-brand-50/40">
                  <td className="py-3">
                    <input
                      ...
                      onChange={() =>
                        setSelectedIds((c) =>
                          c.includes(task.id) ? c.filter((id) => id !== task.id) : [...c, task.id],
                        )
                      }
                      className="h-4 w-4 accent-brand-700"
                    />
                  ...
                  <td className="py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${task.priority === "High" ? "bg-red-100 text-red-800" : task.priority === "Medium" ? "bg-amber-100 text-amber-800" : "bg-slate-100 text-slate-700"}`}
                    >
                      {task.priority}
                    </span>
                  </td>
                  ...
                  <td className="py-3">
                    <select
                      value={task.status}
                      onChange={(e) =>
                        updateStatus(task.id, e.target.value as HousekeepingTask["status"])
                      }
...
function TaskForm({
  ...
  employees: Employee[];
  onSave: (data: Omit<HousekeepingTask, "id"> & { id?: string }) => void;
  onCancel: () => void;
}) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
      ...
      <div className="grid md:grid-cols-3 gap-3">
        ...
        <label className="text-xs text-brand-700">
          ...
          <select
            name="taskType"
            required
            defaultValue={task?.taskType || "Standard Clean"}
            className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
          >
            <option>Standard Clean</option>
            <option>Deep Clean + Turndown</option>
            <option>Check-out Clean</option>
            <option>Linen Refresh</option>
            <option>Mini-bar Restock</option>
            <option>VIP Turndown</option>
    ...
    <div className="bg-white rounded-xl border border-brand-100 p-6">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {[
          ...
          <div
            key={i.n}
            className="flex items-center justify-between p-3 border border-brand-100 rounded-md"
          >
            <div className="font-medium text-brand-900 text-sm">{i.n}</div>
            <span
              className={`text-xs px-2 py-0.5 rounded-full ${i.c === "emerald" ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-700"}`}
            >
              {i.s}
            </span>
          </div>
      ...
      <div className="space-y-3">
        {[
          ...
          >
            ...
            <div
              className={`w-10 h-6 rounded-full relative cursor-pointer ${n.on ? "bg-brand-600" : "bg-slate-300"}`}
            >
              <div
                className={`absolute top-0.5 w-5 h-5 rounded-full bg-white transition ${n.on ? "left-4" : "left-0.5"}`}
              />
            </div>
          </div>
        ))}
    ...
    <div className="bg-white rounded-xl border border-brand-100 p-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <div className="flex gap-2">
            {["#1a3826", "#245435", "#5fa06e", "#bcdcbc", "#b8923f", "#d4af6a"].map((c) => (
              <div
                key={c}
                className="w-12 h-12 rounded-md border border-brand-100"
                style={{ background: c }}
              />
            ))}
          </div>
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-brand-200 rounded-md p-8 text-center text-xs text-brand-500">
      ...
      <div className="grid md:grid-cols-3 gap-4 text-sm">
        <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-100">
          ...
          <div className="text-xs text-emerald-700">
            Payment card data encrypted at rest & in transit
          </div>
        </div>
        <div className="p-4 bg-emerald-50 rounded-lg border border-emerald-100">
...
function GuestsTab() {
  ...
  const filtered = guests.filter((g) =>
    [g.name, g.email, g.phone, g.country].some((f) =>
  ...
  const inviteUrl =
    typeof window !== "undefined" ? `${window.location.origin}/guest-login` : "/guest-login";
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <StatCard
          label="Platinum"
          value={String(guests.filter((g) => g.loyaltyTier === "Platinum").length)}
          icon={<Sparkles className="w-5 h-5" />}
        />
        <StatCard
          label="Gold"
          value={String(guests.filter((g) => g.loyaltyTier === "Gold").length)}
          icon={<Sparkles className="w-5 h-5" />}
        />
        <StatCard
          label="Total Points"
          value={guests.reduce((s, g) => s + g.points, 0).toLocaleString()}
          icon={<ClipboardList className="w-5 h-5" />}
        />
      </div>
        ...
        <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
          <div>
            ...
            <p className="text-sm text-brand-700">
              Create, edit, and remove guest profiles in the CRM.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <input
        ...
        {guests.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-brand-600 max-w-md mb-6">
              Guest profiles will appear here once they register through the guest portal or when
            ...
            <div className="flex items-center gap-3 flex-wrap justify-center">
              ...
              <Link
                to="/guest-login"
                className="px-4 py-2 border border-brand-200 text-brand-700 rounded-md text-sm hover:bg-brand-50 flex items-center gap-2"
              >
              ...
              <div className="mt-6 w-full max-w-sm p-4 rounded-lg bg-brand-50 border border-brand-100 text-left">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 border border-brand-200 rounded-md text-xs hover:bg-white text-brand-700"
                  >
                    Cancel
                  </button>
                </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-brand-500">
                      No guests found.
                    </td>
                  </tr>
                )}
                {filtered.map((g) => (
                  <tr key={g.id} className="border-b border-brand-50 hover:bg-cream-50/50">
                    ...
                    <td className="py-3">
                      <span className={`px-2 py-1 rounded text-xs ${tierBadge(g.loyaltyTier)}`}>
                        {g.loyaltyTier}
                      </span>
                    </td>
                    ...
                    <td className="py-3 text-right">
                      <div className="inline-flex items-center gap-1">
                        <button
                          ...
                          className="p-2 hover:bg-brand-100 rounded text-brand-700"
                          title="Edit"
                        >
      ...
      {(creating || editing) && (
        <GuestForm
          ...
          onSave={async (g) => {
            const res = editing
              ? await updateGuestProfile2(editing.id, g)
              : await addGuestProfile(g as Guest);
            addToast(
              res.ok ? "success" : "error",
              res.ok ? (editing ? "Guest updated" : "Guest created") : "Save failed",
              res.error,
            );
            if (res.ok) {
              setCreating(false);
...
function GuestForm({
  ...
  const setPwd = async (opts: { data: { email: string; password: string } }) => {
    throw new Error(
      "Admin password management requires a server component. Not available in this build.",
    );
  };
    ...
    >
      ...
      >
        <div className="p-6 border-b border-brand-100 flex items-center justify-between">
          ...
          <button type="button" onClick={onCancel} className="p-1 hover:bg-brand-100 rounded">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <label className="text-xs text-brand-700">
            ...
            <select
              ...
              onChange={(e) =>
                setDraft({ ...draft, loyaltyTier: e.target.value as Guest["loyaltyTier"] })
              }
              className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
            >
              {TIERS.map((t) => (
          ...
          <label className="text-xs text-brand-700">
            ...
            <input
              type="number"
              min="0"
              value={draft.totalStays ?? 0}
              onChange={(e) => setDraft({ ...draft, totalStays: Number(e.target.value) })}
              className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
            />
          </label>
          <label className="text-xs text-brand-700">
            ...
            <input
              ...
              min="0"
              step="0.01"
              value={draft.totalSpent ?? 0}
              onChange={(e) => setDraft({ ...draft, totalSpent: Number(e.target.value) })}
              className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
            />
          </label>
          <label className="text-xs text-brand-700">
            ...
            <input
              type="date"
              value={draft.lastStay ?? ""}
              onChange={(e) => setDraft({ ...draft, lastStay: e.target.value })}
              className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
            />
          </label>
          <label className="text-xs text-brand-700 md:col-span-2">
            ...
            <input
              ...
              placeholder="Ocean view, Vegetarian, Late checkout"
              className="mt-1 w-full px-3 py-2 border border-brand-200 rounded-md bg-white text-sm"
            />
          </label>
        </div>
          ...
          <div className="px-6 pb-6">
            <div className="border border-brand-200 rounded-lg p-4 bg-cream-50/50">
              ...
              <p className="text-xs text-brand-600 mb-3">
                Sets a new password for the auth account matching{" "}
                <strong>{guest.email || "—"}</strong>. Minimum 8 characters.
              </p>
              <div className="flex gap-2">
                <input
                  type="text"
        ...
        <div className="p-6 border-t border-brand-100 flex justify-end gap-2">
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2 text-sm border border-brand-200 rounded-md hover:bg-brand-50"
          >
            Cancel
          </button>
          <button
            ...
            className="px-4 py-2 bg-brand-800 text-cream-50 rounded-md text-sm hover:bg-brand-900 flex items-center gap-2"
          >
            <Save className="w-4 h-4" /> {guest ? "Update" : "Create"}
          </button>
        </div>
      </form>
    </div>
...
function RolesTab() {
  ...
  const updateCreds = async (opts: {
    data: { userId: string; email?: string; password?: string };
  }) => {
    throw new Error(
      "Admin credential management requires a server component. Not available in this build.",
    );
  };
  ...
  async function saveCreds(u: UserRow) {
    ...
    try {
      ...
      if (emailChanged) {
        setUsers((prev) =>
          prev.map((x) => (x.id === u.id ? { ...x, email: editEmail.trim() } : x)),
        );
      }
      cancelEdit();
    } catch (e: any) {
  ...
  useEffect(() => {
    if (isAdmin) loadUsers(); /* eslint-disable-next-line */
  }, [isAdmin]);
  ...
  async function toggleRole(u: UserRow, role: AppRole, has: boolean) {
    ...
    try {
      if (has) {
        ...
        const { error } = await supabase.from("user_roles").insert({ user_id: u.id, role } as any);
        if (error) throw error;
      }
      setUsers((prev) =>
        prev.map((x) =>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-brand-700 mt-1">
              Grant <strong>employee</strong> to allow front-desk &amp; reservation work, or{" "}
              <strong>admin</strong> for full access. Guests use the guest portal only.
            </p>
          </div>
          <div className="flex gap-2">
            ...
            <button
              onClick={loadUsers}
              className="px-4 py-2 bg-brand-800 text-cream-50 rounded-md text-sm hover:bg-brand-900"
            >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {roleList.map((r) => {
                          ...
                          return (
                            ...
                            >
                              {has ? <Check className="w-3 h-3" /> : <Plus className="w-3 h-3" />}{" "}
                              {r}
                            </button>
                          );
                        })}
      ...
      <div className="text-xs text-brand-500">
        Tip: have the user sign up first via the staff login page, then return here to grant them
        the <em>employee</em> role.
      </div>
    </div>
  );
}
